...
func printTreeHelperBidir(node *Nodebidir, prefix string, isLast bool) {
	...
	for i, recipe := range node.combinations {
		...
		printTreeHelperBidir(recipe.ingredient1, combinationChildPrefix, false) // Ingredient 1 is never the absolute last in its group combo print
		printTreeHelperBidir(recipe.ingredient2, combinationChildPrefix, true)  // Ingredient 2 is always the last in its group combo print (for this recipe)
	}
}
